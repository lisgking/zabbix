...
EOF;
        ...
        foreach ($column_titles as $index => $val)
        {
            ...
            if(preg_match('/no_order/',$index)){
            	$HTML .= $val;
            }elseif( ( isset($this->idsadmin->in['orderby'])==$index) && !(isset($this->idsadmin->in['orderway'])) ) {
...
return $HTML;
    ...
    function sysgentab_row_output($data)
    {
        ...
        foreach ($data as $index => $val)
        {
            ...
            {
            	case "status":
            		if (trim(strtolower($val)) == "online") 
            		{
            			$HTML .= "<td><font color='green'>";
		            	$HTML .= $this->idsadmin->lang($val);
		            	$HTML .= "</font></td>";
            		} else {
	            		$HTML .= "<td><font color='red'><strong>";
			            $HTML .= $this->idsadmin->lang($val);
			            $HTML .= "</strong></font></td>";
            		}
            		break;
