    ...
    function sysgentab_start_output( $title, $column_titles, $pag="")
    {
        ...
        <script type="text/javascript">
            ...
            function doMessage(what)
            {
               ...
               "<img class='dialogimg' src='images/warning.png' alt='{$this->idsadmin->lang('Warning')}' align='middle'></img><div align='center'>"+msgStr+"<br/><br/><div align='right'><button type='dijit.form.Button' onclick='doDelete(\""+what+"\")'>{$this->idsadmin->lang('Yes')}</button>&nbsp;&nbsp;<button type='dijit.form.Button' onclick='closeDialog()'>{$this->idsadmin->lang('No')}</button></div></div>"
               );
               dialogConfirm.show();   
            }
            ...
            console.log(refresh);
               ...
               if ( refresh == 1 )
               {
                window.location="index.php?act=show&do=showBarActLogTail";
               }
            }
...
<div dojoType="dijit.layout.ContentPane" style="height:450px; overflow: auto; border-bottom: 1px solid #9BABC5;">
