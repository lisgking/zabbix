...
-- Red Alarm - Left check value       -- %param1%
         "execute function hadv_stage_data('intvl type caller cp_time','select intvl,type,caller,cp_time from sysmaster:syscheckpoint where caller=""plog"" and block_time > 0','t_red_ckpt_caller_plog')", 
...
-- Yellow Alarm - Left check value    -- %param1% 
         "execute function hadv_stage_data('intvl type caller cp_time','select intvl,type,caller,cp_time from sysmaster:syscheckpoint where caller=""plog"" ','t_yel_ckpt_caller_plog')", 
