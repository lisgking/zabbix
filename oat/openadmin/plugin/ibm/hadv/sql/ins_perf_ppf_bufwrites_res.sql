...
-- Red Alarm - Left check value       -- %param1%
	"execute function hadv_stage_data_exception('dbsname tabname partnum bufwrites total_bufwrites', 'select t1.dbsname ,t1.tabname ,t1.partnum ,t1.bufwrites , (select sum(t2.bufwrites) from sysmaster:sysptprof t2 where t2.dbsname not in (""sysmaster"",""sysutils"" ,""system"",""sysuser"",""sysadmin"")  AND t2.tabname != ""TBLSpace"") total_bufwrites from sysmaster:sysptprof t1 where t1.dbsname not in (""sysmaster"",""sysutils"",""system"",""sysuser"","" sysadmin"") AND t1.tabname != ""TBLSpace"" group by 1,2,3,4 having trunc( (t1.bufwrites / (select sum(t2.bufwrites) from sysmaster:sysptprof t2 where t2.dbsname not in (""sysmaster"",""sysutils"" ,""system"",""sysuser"",""sysadmin"")  AND t2.tabname != ""TBLSpace"") * 100),0) > %lparam1%' ,'t_red_ppf_bufwrites','partnum',%v_prof_id%)",
...
-- Yellow Alarm - Left check value    -- %param1% 
	"execute function hadv_stage_data_exception('dbsname tabname partnum bufwrites total_bufwrites', 'select t1.dbsname ,t1.tabname ,t1.partnum ,t1.bufwrites , (select sum(t2.bufwrites) from sysmaster:sysptprof t2 where t2.dbsname not in (""sysmaster"",""sysutils"" ,""system"",""sysuser"",""sysadmin"")  AND t2.tabname != ""TBLSpace"") total_bufwrites from sysmaster:sysptprof t1 where t1.dbsname not in (""sysmaster"",""sysutils"",""system"",""sysuser"","" sysadmin"") AND t1.tabname != ""TBLSpace"" group by 1,2,3,4 having trunc( (t1.bufwrites / (select sum(t2.bufwrites) from sysmaster:sysptprof t2 where t2.dbsname not in (""sysmaster"",""sysutils"" ,""system"",""sysuser"",""sysadmin"")  AND t2.tabname != ""TBLSpace"") * 100),0) > %lparam1%' ,'t_yel_ppf_bufwrites','partnum',%v_prof_id%)",
