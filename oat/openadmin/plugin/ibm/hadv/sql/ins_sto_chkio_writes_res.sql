...
-- Red Alarm - Left check value       -- %param1%
         "execute function hadv_stage_data_exception('dbspace chknum pageswritten total_written write_perc',' select t3.name dbspace, t1.chknum, t1.pageswritten, sum(t2.pageswritten) total_written, trunc((t1.pageswritten / sum(t2.pageswritten) * 100),0) write_perc from sysmaster:syschktab t1 , sysmaster:syschktab t2,sysmaster:sysdbspaces t3 where t3.dbsnum=t1.dbsnum group by t3.name,t1.chknum,t1.pageswritten having trunc((t1.pageswritten / sum(t2.pageswritten) * 100),0) > %lparam1% ', 't_red_sto_chkio_writes','dbspace',%v_prof_id%)",
...
-- Yellow Alarm - Left check value    -- %param1% 
         "execute function hadv_stage_data_exception('dbspace chknum pageswritten total_write write_perc',' select t3.name dbspace, t1.chknum, t1.pageswritten, sum(t2.pageswritten) total_write, trunc((t1.pageswritten / sum(t2.pageswritten) * 100),0) write_perc from sysmaster:syschktab t1 , sysmaster:syschktab t2,sysmaster:sysdbspaces t3 where t3.dbsnum=t1.dbsnum group by t3.name,t1.chknum,t1.pageswritten having trunc((t1.pageswritten / sum(t2.pageswritten) * 100),0) > %lparam1% ', 't_yel_sto_chkio_writes','dbspace',%v_prof_id%)",
